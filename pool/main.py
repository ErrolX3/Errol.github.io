...
class ListHandler(webapp2.RequestHandler):
  ...
  class PostHandler(object):
 	def get(self):
		...
		self.response.write(template.render())
	def post(self):
		text = self.request.get('postText')
        new_post = Post(text=text)
        new_post.put()
        template = jinja_environment.get_template('posted.html')
        self.response.write(template.render(
            {
              'text': text
            }))
...
class SinglePHandler(webapp2.RequestHandler):
    def get(self):
    	post_id=self.request.get('id')
    	post_id=int(post_id)
    	single_post = Post.get_by_id(post_id)
    	template = jinja_environment.get_template('newlist.html')
    	self.response.write(template.render({
...
class RecentHandler(webapp2.RequestHandler):
	def get(self):
		posts_query = post.query().order(-Post.timestamp)
        list_of_posts = posts_query.fetch(limit=10)
